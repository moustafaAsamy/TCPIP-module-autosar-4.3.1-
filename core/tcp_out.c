                      ...
                      u32_t seqno_be /* already in network byte order */)
{
  ...
  if (p != NULL) {
    ...
    UARTprintf("Transmit :>  ackno =  %d   ,  seqno =  %d  ,   len  =  %d \n", tcphdr->ackno, tcphdr->seqno , datalen);
    //UARTprintf( " %d            =    %d        \n " , tcphdr->ackno ,pcb->rcv_nxt);
...
tcp_fast_rexmit(struct tcp_pcb *pcb)
{
...
tcp_send_empty_ack(struct tcp_pcb *pcb)
{
  ...
  struct tcp_hdr *tcphdr;
  u8_t optlen = 0;
  p = tcp_output_alloc_header(pcb, optlen, 0, (pcb->snd_nxt));
  if (p == NULL) {
...
tcp_output(struct tcp_pcb *pcb)
{
  ...
  if (pcb->flags & TF_ACK_NOW &&   (seg == NULL || (seg->tcphdr->seqno) - pcb->lastack + seg->len > wnd)) // will  only check first condtion if it is right
   ...
   */
 u8_t o= (seg->tcphdr->seqno) -(pcb->lastack +seg->len);// the sequence no. is greater than the last ack + your len.
  //while ( (seg != NULL) &&   (((seg->tcphdr->seqno) -(pcb->lastack +seg->len)) <= wnd) )
  while (  seg != NULL &&                ((seg->tcphdr->seqno) -     pcb->lastack +    seg->len )<= wnd   ) // loop to send all unsent ,  make sure that you are sending in the window
 {
    pcb->unsent = seg->next;
...
static void tcp_output_segment(struct tcp_seg *seg, struct tcp_pcb *pcb)
{
  ...
  {
        seg->tcphdr->chksum = 0;
        seg->tcphdr->chksum = inet_chksum_pseudo(seg->p, &(pcb->local_ip),&(pcb->remote_ip),IP_PROTO_TCP, seg->len + PBUF_TRANSPORT_HLEN  );
        seg->p->free_buffer =0;
        ip_output_autosar(seg->p, &(pcb->local_ip), &(pcb->remote_ip), pcb->ttl, pcb->tos,IP_PROTO_TCP,sockets_list[pcb->socket_ID].TcpIpAddrId);
        UARTprintf("Transmit :>  ackno =  %d   ,  seqno =  %d  ,   len  =  %d \n", seg->tcphdr->ackno, seg->tcphdr->seqno , seg->len);
  }
  else
  {
        seg->tcphdr->chksum = 0;
